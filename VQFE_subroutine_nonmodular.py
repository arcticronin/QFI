...
L = 2
# Before calling minimize, flatten the 3D parameter array:
init_params = 0.1 * np.random.randn(L, n_qubits, 3)
init_params_flat = init_params.flatten()


# Inside the cost function, reshape the flat array back:
def cost_flat(flat_params, theta):
    params = flat_params.reshape(L, n_qubits, 3)
    return cost(params, theta)


# Now call minimize using the flat version
result = minimize(
